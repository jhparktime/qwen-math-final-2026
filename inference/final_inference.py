...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--adapter", type=Path, required=True, help="Frozen R2 LoRA directory")
...
def main() -> None:
    ...
    adaptive_candidates: dict[str, list[dict[str, object]]] = {}
    ...
    for qid in frame["id"].astype(str):
        ...
        if qid in records_4096:
            long_4096 = sorted(records_4096[qid]["candidates"], key=lambda item: int(item["sample_index"]))
            long_8192 = (
                sorted(records_8192[qid]["candidates"], key=lambda item: int(item["sample_index"]))
